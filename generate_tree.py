...
script_dir = os.path.split(my_path)[0]
...
def generate_simple_tree(tree_title, tree_model, X, target_names,
	...
	# get template
	print(script_dir)
	path1 = os.path.join(script_dir, 'src/simple_tree_template.html')
	print(path1)
	temp = open(path1).read() 
	...
	path2 = os.path.join(script_dir, 'simple_tree_output/simple_tree_%s.html' %(tree_title))
	print(path2)
	with open(path2, 'wb') as fh:
		...
		print('The Decistion Tree visualisation output is in %s.' %(path2))
...
def generate_sankey_tree(tree_title, tree_model, X, target_names,
	...
	# get template
	print(script_dir)
	path3 = os.path.join(script_dir, 'src\sankey_tree_template.html')
	print(path3)
	temp = open(path3).read()
	template = jinja2.Template(temp)
	...
	# generate output html
	path4 = os.path.join(script_dir, 'sankey_tree_output\sankey_tree_%s.html' %(tree_title))
	print(path4)
	with open(path4, 'wb') as fh:
		...
		fh.write(template.render(render_result))
		print('The Sankey Tree visualisation output is in %s.' %(path4))
